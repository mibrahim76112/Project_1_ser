...
def sample_train_and_test(train_ts, type_model):
    ...
    for i in sorted(train_ts['faultNumber'].unique()):
        if i == 0:
            frames_test.append(train_ts[train_ts['faultNumber'] == i].iloc[20000:22000])
        else:
            fr = []
            b = train_ts[train_ts['faultNumber'] == i]
...
def scale_and_window(X_df, scaler, y_col='faultNumber', window_size=20, stride=5):
    """
    ...
    """
    y = X_df[y_col].values
    X = X_df.iloc[:, 3:].values  # Drop meta columns

    X_scaled = scaler.transform(cp.asarray(X))  # GPU scaling
    ...
    X_win = X_scaled[X_indices]         # Shape: [num_windows, window_size, num_features]
    y_win = cp.asarray(y)[y_indices]    # Shape: [num_windows]
...
def load_sampled_data(window_size=20, stride=5, type_model="supervised"):
    ...
    print(f"[INFO] Scaler fit on fault-free samples: {fault_free.shape[0]} rows")
